...
$PAGE->set_url('/mod/englishcentral/index.php', ['id' => $id]);
...
if ($englishcentrals = get_all_instances_in_course('englishcentral', $course)) {
    ...
    if ($usessections) {
        ...
        $table->head  = [$sectionname, get_string('name')];
    ...
    foreach ($englishcentrals as $englishcentral) {
        $params = ['id' => $englishcentral->coursemodule];
        ...
        if ($usessections) {
            ...
            $table->data[] = [$englishcentral->section, $link];
        } else {
            $table->data[] = [$link];
        }
    }
